...
int main(int argc, char *argv[])
{
	...
	if(candyRanking.is_open()) 
	{
		...
		while(getline(candyRanking, line))
		{
			...
			arr[0] = strtok(str, "|"); 
			arr[1] = strtok(NULL,"|");
			CandyMap.insert(make_pair(atoi(arr[1]), arr[0]));
			delete[] str;
		}
		candyRanking.close();
	}
	...
	if (houseNames.is_open())
	{
		...
		while(getline(houseNames,line))
		{
			...
			randomhouse = rand() % 2 + 1; 
			//randomly choose number 1 - 2
			if ( randomhouse == 1)
			{
				HVec.push_back(new CandyHouse(line,CandyMap));
			}
			else 
			{
				HVec.push_back( new ToothbrushHouse(line,CandyMap));
			}
